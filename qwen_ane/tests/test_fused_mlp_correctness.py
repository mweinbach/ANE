...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Correctness test for fused ANE SwiGLU-FFN kernel")
...
def main() -> int:
    ...
    kern = bridge.compile_fused_swiglu_ffn(w1, w3, w2, spatial=args.spatial)
    compile_ms = (time.perf_counter() - t0) * 1e3
    print(
        f"compiled fused kernel dim={dim} hidden={hidden} spatial={args.spatial} "
